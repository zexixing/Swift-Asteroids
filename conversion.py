...
def num_assu(wvm_name, aperture, start=False,profile=False):
    if not start:
        start = 0.
...
def num2q_fit(aper_list, num_list, num_err_list, wvm_name, 
              aperture=False, if_show = True, start=False):
    # get assumed num of the model within the aperture
    ...
    para,para_err=optimize.curve_fit(func,aper_list,num_list,[1])
    # ratio -> actual Q_H2O
    q = q_assu*para[0]
    q_err = q_assu*para_err[0][0] #TODO:
    if if_show == True:
        print('water production rate: '+str(q)+' +/- '+str(q_err))
    return q, q_err, dis2col, para[0]
